...
class ProblemDTLZ4():
	...
	@staticmethod
	def DTLZ4_generateRandomSolution(n, ndiv = 0, index = 0):
		retVal = []
		if ndiv == 0:
			for _ in range(n):
	...
	# DTLZ4_f son las funciones objetivo definidas, para el DTLZ4 es como sigue:
	# Min f_1(X) =  (1 + g(X_M)) * cos((x_1 ** alpha) * PI_HALF ) * ... * cos((x_{M-1} ** alpha) * PI_HALF)
	# Min f_1(X) =  (1 + g(X_M)) * cos((x_1 ** alpha)* PI_HALF ) * ... * sin((x_{M-1} ** alpha) * PI_HALF)
	# Min   .						 .
	# Min   .						 .
	# Min   .						 .
	...
	def DTLZ4(self, X,M):
		...
		for i in range(1,M+1):
			...
			solution.append(self.DTLZ4_f(i,X,M))
		return solution
